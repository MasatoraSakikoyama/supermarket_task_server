...
Redis is used for storing user session tokens. Configure a Redis instance with the following:
- Host: `localhost` (or your Redis server address)
- Port: `6379` (default Redis port)
- Database: `0` (default database)
- Password: (optional, if your Redis instance requires authentication)

Tokens are stored with automatic expiration based on `JWT_ACCESS_TOKEN_EXPIRE_MINUTES`.
